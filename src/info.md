...
- `seo_auditor/reporters.py`: exportación a JSON, Excel, Word, PDF, HTML y Markdown, incluyendo capa semántica intermedia única para alinear DOCX/PDF/HTML, bloque `meta` completo (cliente/gestor/fecha_ejecucion/periodo/sitemap), bloques semánticos explícitos (`metadatos`, `kpi_cards`, `resumen_ejecutivo`, `prioridades`, `tablas_detalle`) con compatibilidad legacy (`parrafos/tablas/tarjetas`), síntesis ejecutiva arriba + detalle abajo, sanitización editorial por formato y consolidación de hoja `Contenido` por URL con métricas agregadas (conteos por severidad/tipo, áreas afectadas y señales clave on-page) manteniendo la trazabilidad de incidencias en `Errores`.
