...
def _renderizar_bloque_dashboard(
    hoja,
    celda_inicio: str,
    titulo: str,
    lineas: list[str],
    color_bloque: str,
    columnas_ancho: int = 6,
) -> int:
...
def _construir_quick_wins(filas: list[dict], limite: int = 7) -> list[dict[str, object]]:
    ...
    for fila in filas:
        ...
        if url not in agrupados:
            ...
            agrupados[url] = {
                ...
                "problemas": [],
                "recomendaciones": [],
                "impacto_maximo": impacto or "Medio",
                "esfuerzo_maximo": esfuerzo or "Medio",
                "prioridad": 0,
            }
        ...
        tarjeta["prioridad"] = escala_impacto.get(str(tarjeta["impacto_maximo"]), 0) * 10 - escala_esfuerzo.get(
            str(tarjeta["esfuerzo_maximo"]), 99
        )
    ...
    quick_wins_ordenados = sorted(
        quick_wins, key=lambda item: (-int(item.get("prioridad", 0)), str(item.get("url", "")))
    )
...
def construir_secciones_desde_ia(texto_ia: str | None) -> list[dict[str, object]]:
    ...
    for linea in texto_limpio.splitlines():
        ...
        if re.match(
            (
                r"^(\d+[\).]\s*)?"
                r"(resumen|kpis|hallazgos|quick wins|acciones técnicas|"
                r"acciones de contenido|rendimiento|roadmap)"
            ),
            linea.lower(),
        ):
    ...
    for item in resultado.rendimiento:
        ...
        if not item.oportunidades:
            ...
            fila.update(
                {
                    "oportunidad": "",
                    "descripcion": item.error or "Sin oportunidades destacadas",
                    "ahorro_estimado": "",
                    "severidad": "informativa",
                    "recomendacion": "Mantener monitorización continua",
                    "observaciones": item.error or "",
                }
            )
            ...
            fila.update(
                {
                    ...
                    "ahorro_estimado": oportunidad.ahorro_estimado,
                    "severidad": oportunidad.severidad,
                    "recomendacion": f"Aplicar mejora: {oportunidad.titulo}",
                    "observaciones": "",
                }
            )
...
def construir_cruces_gsc_analytics(resultado: ResultadoAuditoria) -> list[dict]:
    ...
    gsc_por_url = {
        _clave_url_cruce(item.url): item for item in resultado.search_console.paginas if _clave_url_cruce(item.url)
    }

    # Construye índice de Analytics por URL.
    analytics_por_url = {
        _clave_url_cruce(item.url): item for item in resultado.analytics.paginas if _clave_url_cruce(item.url)
    }
    ...
    indice_indexacion = {
        _clave_url_cruce(item.url): item for item in resultado.gestion_indexacion if _clave_url_cruce(item.url)
    }
    ...
    for url in urls_cruzables:
        ...
        if (
            metrica_analytics.sesiones >= CRUCE_GA_MIN_SESIONES_REBOTE
            and metrica_analytics.rebote >= CRUCE_GA_MIN_REBOTE_ALTO
        ):
        ...
        if (
            metrica_gsc.impresiones >= CRUCE_GSC_MIN_IMPRESIONES_SIN_ENGAGEMENT
            and metrica_analytics.sesiones <= CRUCE_GA_MAX_SESIONES_SIN_ENGAGEMENT
        ):
        ...
        if (
            metrica_gsc.impresiones >= OPORTUNIDAD_GSC_MIN_IMPRESIONES_CTR
            and metrica_gsc.ctr <= OPORTUNIDAD_GSC_MAX_CTR
        ):
        ...
        if (
            decision_indexacion
            and decision_indexacion.clasificacion == "NO_INDEXAR"
            and metrica_analytics.conversiones > 0
        ):
...
def calcular_score_prioridad_pagina(gsc: object | None, ga: object | None, tecnico: object | None) -> dict[str, object]:
    ...
    if (
        ga
        and ga.sesiones >= PRIORIDAD_GA_MIN_SESIONES_SIN_CONVERSION
        and ga.conversiones <= PRIORIDAD_GA_MAX_CONVERSIONES
    ):
    ...
    if (
        gsc
        and ga
        and gsc.impresiones >= PRIORIDAD_GSC_MIN_IMPRESIONES_ENGAGEMENT_BAJO
        and ga.rebote >= PRIORIDAD_GA_MIN_REBOTE_BAJO_ENGAGEMENT
    ):
...
def construir_paginas_prioritarias(resultado: ResultadoAuditoria, limite: int = 10) -> list[dict]:
    ...
    tecnico_por_clave = {
        _clave_url_cruce(item.url): item for item in resultado.resultados if _clave_url_cruce(item.url)
    }
    ...
    return sorted(
        prioridades,
        key=lambda item: (-float(item["prioridad_score"]), -float(item["impresiones"]), -float(item["sesiones"])),
    )[:limite]
...
def construir_oportunidades_gsc(resultado: ResultadoAuditoria) -> list[dict]:
    ...
    for metrica in datos_gsc.paginas:
        ...
        # Clasifica oportunidad por posición de crecimiento rápido.
            ...
            OPORTUNIDAD_GSC_MIN_POSICION <= metrica.posicion_media <= OPORTUNIDAD_GSC_MAX_POSICION
            and metrica.impresiones >= OPORTUNIDAD_GSC_MIN_IMPRESIONES_POSICION
        ):
        ...
        esfuerzo = (
            "Bajo"
            if problema_onpage in {"meta description ausente", "title mejorable", "headings deficientes"}
            else "Medio"
        )
...
def _calcular_metricas_gsc(filas_gsc_paginas: list[dict]) -> dict[str, float]:
    ...
    posicion_media = (
        round(
            sum(float(fila.get("posicion_media", 0.0)) for fila in filas_gsc_paginas) / max(1, len(filas_gsc_paginas)),
            2,
        )
...
def calcular_metricas(resultado: ResultadoAuditoria) -> dict[str, int | float | dict[str, int] | str]:
    ...
    score_indexacion = round(
        max(5.0, min(100.0, 100.0 - ((areas.get("Indexación", 0) + areas.get("Arquitectura", 0)) * 4.0))), 1
    )
    ...
    scores_rendimiento = [
        item.performance_score for item in resultado.rendimiento if isinstance(item.performance_score, (int, float))
    ]

    # Calcula score de rendimiento con fallback al global si no hay datos.
    score_rendimiento = (
        round(sum(scores_rendimiento) / len(scores_rendimiento), 1)
        if scores_rendimiento
        else float(resultado.score_rendimiento or score)
    )

    # Resuelve score técnico preferente desde resultado consolidado.
    score_tecnico = (
        float(resultado.score_tecnico) if isinstance(resultado.score_tecnico, (int, float)) else score_indexacion
    )

    # Resuelve score de contenido preferente desde resultado consolidado.
    score_contenido_final = (
        float(resultado.score_contenido) if isinstance(resultado.score_contenido, (int, float)) else score_contenido
    )
...
def _construir_bloques_narrativos(resultado: ResultadoAuditoria) -> dict[str, list[str]]:
    ...
    if not resumen_tenia_contenido:
        ...
        bloques["Resumen ejecutivo"].append(
            f"Se auditaron {resultado.total_urls} URLs con fuentes activas: {', '.join(resultado.fuentes_activas)}."
        )
    ...
    if not bloques["Visibilidad orgánica real"] and resultado.search_console.activo:
        ...
        bloques["Visibilidad orgánica real"].append(
            f"Clics: {clics_totales} | Impresiones: {impresiones_totales} | "
            f"CTR medio: {ctr_medio} | Posición media: {posicion_media}."
        )
        ...
        for fila in sorted(filas_gsc_paginas, key=lambda item: float(item.get("impresiones", 0.0)), reverse=True)[:5]:
            ...
            bloques["Visibilidad orgánica real"].append(
                f"Top página: {fila.get('url', '')} | "
                f"impresiones={fila.get('impresiones', 0)} | "
                f"clics={fila.get('clics', 0)}."
            )
    ...
    if not bloques["Oportunidades SEO prioritarias"] and resultado.search_console.activo:
        ...
        for fila in oportunidades_gsc[:8]:
            ...
            bloques["Oportunidades SEO prioritarias"].append(
                f"{fila.get('url', '')}: {fila.get('oportunidad', '')} | "
                f"CTR={fila.get('ctr', 0)} | "
                f"posición={fila.get('posicion_media', 0)} | "
                f"acción={fila.get('accion_recomendada', '')}"
            )
    ...
    if not bloques["Hallazgos críticos"]:
        ...
        hallazgos_criticos = [
            fila
            for fila in filas
            if str(fila.get("severidad", "")).lower() in {"crítica", "alta"} and fila.get("problema")
        ]
    ...
    if not bloques["Quick wins"]:
        ...
        quick_wins_indexacion = [
            ...
            if fila.get("clasificacion") in {"NO_INDEXAR", "REVISAR"} and fila.get("prioridad") in {"Alta", "Media"}
        ]
        ...
        for fila in sorted(filas_analytics_paginas, key=lambda item: float(item.get("sesiones", 0.0)), reverse=True):
            ...
            if (
                ...
                bloques["Quick wins"].append(
                    f"[Analytics] Optimizar contenido/CTA en {fila.get('url', '')} por ausencia de conversiones."
                )
    ...
    if not bloques["Acciones técnicas"]:
        ...
        acciones_tecnicas = [
            fila
            for fila in filas
            if fila.get("area") in {"Infraestructura", "Indexación", "Arquitectura"} and fila.get("recomendacion")
        ]
        ...
        if not bloques["Acciones técnicas"]:
            ...
            bloques["Acciones técnicas"].append(
                "Revisar cobertura de rastreo, estado HTTP y canonicals para mantener estabilidad técnica."
            )
    ...
    if not bloques["Rendimiento y experiencia de usuario"]:
        ...
        filas_metricas_validas = [
            fila for fila in filas_rendimiento if isinstance(fila.get("performance_score"), (int, float))
        ]
        ...
        for fila in filas_metricas_validas[:5]:
            ...
            bloques["Rendimiento y experiencia de usuario"].append(
                f"{fila['url']} [{fila['estrategia']}] "
                f"score={fila['performance_score']} "
                f"oportunidad={fila['oportunidad'] or 'sin oportunidad destacada'}"
            )
    ...
    if not bloques["Comportamiento y conversión"] and resultado.analytics.activo:
        ...
        for fila in sorted(filas_analytics_paginas, key=lambda item: float(item.get("sesiones", 0.0)), reverse=True)[
            :ANALYTICS_TOP_TRAFICO_LIMITE
        ]:
            bloques["Comportamiento y conversión"].append(
                f"Top tráfico: {fila.get('url', '')} | "
                f"sesiones={fila.get('sesiones', 0)} | "
                f"rebote={fila.get('rebote', 0)}."
            )

        # Añade páginas con peor rebote.
        for fila in sorted(filas_analytics_paginas, key=lambda item: float(item.get("rebote", 0.0)), reverse=True)[
            :ANALYTICS_PEORES_REBOTE_LIMITE
        ]:
            bloques["Comportamiento y conversión"].append(
                f"Peor rebote: {fila.get('url', '')} | "
                f"rebote={fila.get('rebote', 0)} | "
                f"sesiones={fila.get('sesiones', 0)}."
            )
        ...
        for fila in mejores_engagement:
            bloques["Comportamiento y conversión"].append(
                f"Mejor engagement: {fila.get('url', '')} | "
                f"duración={fila.get('duracion', 0)}s | "
                f"rebote={fila.get('rebote', 0)}."
            )
        ...
        for fila in sorted(filas_analytics_paginas, key=lambda item: float(item.get("sesiones", 0.0)), reverse=True):
            if (
                float(fila.get("sesiones", 0.0)) >= CRUCE_GA_MIN_SESIONES_SIN_CONVERSION
                and float(fila.get("conversiones", 0.0)) <= 0.0
            ):
                bloques["Comportamiento y conversión"].append(
                    f"Tráfico sin conversión: {fila.get('url', '')} | "
                    f"sesiones={fila.get('sesiones', 0)} | "
                    f"conversiones={fila.get('conversiones', 0)}."
                )
    ...
    if not bloques["Indexación y rastreo"]:
        ...
        bloques["Indexación y rastreo"].append(
            f"robots.txt disponible: {'sí' if robots_disponible else 'no'}; URLs bloqueadas: {len(urls_bloqueadas)}."
        )
        ...
        if incoherencias:
            ...
            bloques["Indexación y rastreo"].append(
                "Existen incoherencias entre sitemap y robots: revisar URLs bloqueadas listadas en sitemap."
            )
        else:
            # Añade estado estable cuando no hay incoherencias detectadas.
            bloques["Indexación y rastreo"].append(
                "No se detectaron incoherencias críticas entre sitemap y robots en esta ejecución."
            )
    ...
    if not bloques["Gestión de indexación"]:
        ...
        for fila in filas_por_clasificacion["NO_INDEXAR"][:5]:
            ...
            bloques["Gestión de indexación"].append(
                f"NO_INDEXAR: {fila.get('url', '')} | motivo={fila.get('motivo', '')}."
            )
        ...
        for fila in filas_por_clasificacion["REVISAR"][:5]:
            ...
            bloques["Gestión de indexación"].append(
                f"REVISAR: {fila.get('url', '')} | motivo={fila.get('motivo', '')}."
            )

        # Inserta recomendación clara de ejecución.
        bloques["Gestión de indexación"].append(
            "Recomendación: priorizar exclusión de URLs transaccionales y "
            "corregir señales SEO en URLs con potencial antes de solicitar indexación."
        )
    ...
    if not bloques["Roadmap"]:
        ...
        bloques["Roadmap"].append(
            "Fase 1 (0-30 días): quick wins técnicos con impacto real y "
            "corrección de páginas con impresiones altas y problemas on-page."
        )
        # Añade fase 2 centrada en crecimiento de posiciones y CTR.
        bloques["Roadmap"].append(
            "Fase 2 (31-60 días): optimización de páginas en posición media 4-15, "
            "mejora de CTR y ampliación de contenido con mayor oportunidad."
        )
        # Añade fase 3 centrada en estrategia escalable.
        bloques["Roadmap"].append(
            "Fase 3 (61-90 días): estrategia de contenido, arquitectura "
            "temática, enlazado interno y monitorización continua."
        )
        # Añade fase específica de gobernanza de indexación.
        bloques["Roadmap"].append(
            "Fase 2.5 (45-75 días): implantar gestión de indexación inteligente "
            "con limpieza de URLs no indexables y revisión priorizada de URLs en estado REVISAR."
        )
    ...
    if not contiene_medio_plazo:
        ...
        bloques["Roadmap"].append(
            "Fase Medio Plazo: optimización de Time to Interactive, reducción "
            "de recursos JS/CSS, mejora de LCP mobile y optimización de renderizado."
        )
...
def exportar_excel(resultado: ResultadoAuditoria, path_salida: Path) -> Path:
    ...
    for fila in hoja_errores.iter_rows(
        min_row=2, max_row=max(2, len(filas) + 1), min_col=1, max_col=max(1, len(encabezados))
    ):
    ...
    for fila in hoja_contenido.iter_rows(
        min_row=2, max_row=max(2, len(filas) + 1), min_col=1, max_col=len(columnas_contenido)
    ):
    ...
    for fila in hoja_rendimiento.iter_rows(
        min_row=2, max_row=max(2, len(filas_rendimiento) + 1), min_col=1, max_col=len(columnas_rendimiento)
    ):
    ...
    resumen_gestion_indexacion = (
        metricas.get("gestion_indexacion", {}) if isinstance(metricas.get("gestion_indexacion", {}), dict) else {}
    )
    ...
    top_paginas = (
        ", ".join(
            str(fila.get("url", ""))
            for fila in sorted(filas_gsc_paginas, key=lambda item: float(item.get("impresiones", 0.0)), reverse=True)[
                :3
            ]
        )
        or "Sin datos"
    ...
    top_queries = (
        ", ".join(
            str(fila.get("query", ""))
            for fila in sorted(filas_gsc_queries, key=lambda item: float(item.get("impresiones", 0.0)), reverse=True)[
                :3
            ]
        )
        or "Sin datos"
    ...
    top_paginas_analytics = (
        ", ".join(
            str(fila.get("url", ""))
            for fila in sorted(
                filas_analytics_paginas, key=lambda item: float(item.get("sesiones", 0.0)), reverse=True
            )[:3]
        )
        or "Sin datos"
    )
    ...
    severidades_rend = Counter(
        [str(fila.get("severidad", "informativa")).lower() for fila in filas_rendimiento if fila.get("oportunidad")]
    )

    # Construye distribución por tipo de mejora.
    tipos_mejora = Counter(
        [
            str(fila.get("oportunidad", "Sin clasificar")).strip()
            for fila in filas_rendimiento
            if fila.get("oportunidad")
        ]
    )

    # Construye distribución de severidad técnica.
    severidades_tecnicas = Counter(
        [str(fila.get("severidad", "informativa")).lower() for fila in filas if fila.get("problema")]
    )

    # Construye distribución de incidencias por área para bloque ejecutivo.
    incidencias_por_area = Counter(
        [str(fila.get("area", "Sin área")).strip() for fila in filas if fila.get("problema")]
    )

    # Construye distribución de incidencias por tipo para bloque ejecutivo.
    incidencias_por_tipo = Counter(
        [str(fila.get("tipo", "Sin tipo")).strip() for fila in filas if fila.get("problema")]
    )
    ...
    oportunidades_pos_4_15 = len(
        [fila for fila in filas_oportunidades_gsc if "4-15" in str(fila.get("oportunidad", ""))]
    )
    oportunidades_ctr_bajo = len(
        [fila for fila in filas_oportunidades_gsc if "ctr bajo" in str(fila.get("oportunidad", "")).lower()]
    )
    ...
    _renderizar_bloque_dashboard(
        ...
        [
            (
                f"Por severidad: crítica={incidencias_criticas}, "
                f"alta={incidencias_altas}, media={incidencias_medias}, "
    ...
    # Carga datos del gráfico desde hoja auxiliar.
    grafico_tipos_mejora.add_data(
        Reference(hoja_aux, min_col=8, min_row=1, max_row=max_fila_tipos), titles_from_data=True
    )
...
def construir_modelo_semantico_informe(resultado: ResultadoAuditoria) -> dict[str, Any]:
    ...
    filas_analytics = sorted(
        construir_filas_analytics_paginas(resultado), key=lambda item: float(item.get("sesiones", 0.0)), reverse=True
    )[:10]
    ...
    for titulo_seccion in construir_jerarquia_visible(resultado):
                ...
                bloque["tarjetas"].append(
                    {
                        ...
                        "listas": [
                            {
                                "titulo": "Problemas",
                                "items": [sanitizar_texto_editorial(str(valor)) for valor in item["problemas"]],
                            },
                            {
                                "titulo": "Acciones recomendadas",
                                "items": [sanitizar_texto_editorial(str(valor)) for valor in item["recomendaciones"]],
                            },
                        ],
                    }
                )
        ...
        # Inserta contenido narrativo sanitizado como fallback estándar.
            ...
            bloque["parrafos"] = [
                sanitizar_texto_editorial(str(linea)) for linea in bloques_narrativos.get(titulo_seccion, [])[:10]
            ]
            ...
            listas=[
                {
                    ...
                    "items": [
                        sanitizar_texto_editorial(
                            f"{item.url} | HTTP={item.estado_http} | "
                            f"redirección={'Sí' if item.redirecciona else 'No'} | "
                            f"noindex={'Sí' if item.noindex else 'No'}"
                        )
...
def _renderizar_tabla_pdf(tabla_semantica: dict[str, Any], estilos: dict[str, Any]) -> Table:
    ...
    # Sanea cada celda para evitar markup inválido.
    datos_limpios = [[sanear_texto_para_pdf(str(valor)) for valor in fila] for fila in datos]
...
def exportar_pdf(resultado: ResultadoAuditoria, path_salida: Path) -> Path:
    ...
    estilo_portada = ParagraphStyle(
        name="Portada", parent=estilos["Title"], alignment=1, textColor=colors.HexColor("#1F4E78")
    )
...
def exportar_html(resultado: ResultadoAuditoria, path_salida: Path) -> Path:
    ...
    for seccion in modelo["secciones"]:
        ...
        for tabla in seccion.get("tablas", []):
            ...
            filas = "".join(
                f"<tr>{''.join(f'<td>{escape(sanitizar_texto_editorial(str(valor)))}</td>' for valor in fila)}</tr>"
                for fila in tabla.get("filas", [])
            )
            colspan = max(1, len(tabla.get("columnas", [])))
            fila_vacia = f'<tr><td colspan="{colspan}">No disponible</td></tr>'
            bloques_html.append(
                f"<h3>{escape(str(tabla.get('titulo', 'Tabla')))}</h3>"
                f"<table><thead><tr>{columnas}</tr></thead>"
                f"<tbody>{filas or fila_vacia}</tbody></table>"
            )
        for tarjeta in seccion.get("tarjetas", []):
            listas = "".join(
                (
                    f"<p><b>{escape(str(lista.get('titulo', '')))}</b></p><ul>"
                    + "".join(
                        f"<li>{escape(sanitizar_texto_editorial(str(item)))}</li>" for item in lista.get("items", [])
                    )
                    + "</ul>"
                )
                for lista in tarjeta.get("listas", [])
            ...
            bloques_html.append(
                "<div class='tarjeta'>"
                f"<h3>{escape(sanitizar_texto_editorial(str(tarjeta.get('titulo', ''))))}</h3>"
                f"<p>{escape(sanitizar_texto_editorial(str(tarjeta.get('subtitulo', ''))))}</p>"
                f"{listas}</div>"
            )
        if seccion.get("parrafos"):
            bloques_html.append(
                "<ul>"
                + "".join(
                    f"<li>{escape(sanitizar_texto_editorial(str(linea)))}</li>" for linea in seccion.get("parrafos", [])
                )
                + "</ul>"
            )
        if seccion.get("notas"):
            bloques_html.append(
                "".join(f"<p>{escape(sanitizar_texto_editorial(str(nota)))}</p>" for nota in seccion.get("notas", []))
            )
        secciones_html.append(
            f"<div class='bloque'><h2>{escape(str(seccion['titulo']))}</h2>{''.join(bloques_html)}</div>"
        )
...
<body>
  ...
  <p><b>Cliente:</b> {resultado.cliente} | <b>Fecha:</b> {resultado.fecha_ejecucion} | <b>Gestor:</b> {
        resultado.gestor
    }</p>
  <h2>KPIs</h2>
  <div class="kpis">
